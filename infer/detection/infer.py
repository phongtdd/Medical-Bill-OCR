...
def enlarge_box(xyxy, scale=0.35, img_shape=None):
    x1, y1, x2, y2 = xyxy
    h = y2 - y1
    pad_w = h * scale
...
def run(img_path):
    ...
    model = YOLO("D:/20242/CV/kqua/v8m.pt")
    results = model(img, device='cuda', iou=0.4)
    result = results[0]

    for i, box in enumerate(result.boxes):
        x1, y1, x2, y2 = box.xyxy[0].tolist()
        x1, y1, x2, y2 = enlarge_box([x1, y1, x2, y2], scale=0.35, img_shape=img.shape)
        crop = img[y1:y2, x1:x2]
        save_path = os.path.join(output_folder, f"crop_{i+1}.jpg")
        cv2.imwrite(save_path, crop)

    print(f"✅ Saved {i+1} crops to '{output_folder}/'")
